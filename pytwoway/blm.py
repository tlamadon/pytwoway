...
class BLMVarianceDecomposition:
    ...
    def _fit(self, jdata, sdata, fe_params, fe_params_comp, no_controls, gj=None, gs=None, pk1=None, pk0=None, uncorrelated_errors=False, complementarities=True, worker_types_as_ids=True, weighted=True, rng=None):
        '''
        Interior method to fit().
    ...
    def fit(self, jdata, sdata, n_samples=5, reallocate=False, reallocate_jointly=True, reallocate_period='first', uncorrelated_errors=False, Q_var=None, Q_cov=None, complementarities=True, firm_clusters_as_ids=True, worker_types_as_ids=True, weighted=True, ncore=1, rng=None):
        ...
        if ncore > 1:
            # Multiprocessing
            with Pool(processes=ncore) as pool:
                res_lst = list(tqdm(pool.imap(tw.util.f_star, [(self._fit, (jdata, sdata, fe_params, fe_params_comp, no_controls, gj, gs, pk1, pk0, uncorrelated_errors, complementarities, worker_types_as_ids, weighted, np.random.default_rng(seed))) for seed in seeds]), total=n_samples))
                # sim_model_lst = pool.starmap(self._fit, tqdm([(jdata, sdata, fe_params, fe_params_comp, no_controls, gj, gs, pk1, pk0, uncorrelated_errors, complementarities, worker_types_as_ids, weighted, np.random.default_rng(seed)) for seed in seeds], total=n_samples))
        else:
            # No multiprocessing
