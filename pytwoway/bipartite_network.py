...
class BipartiteLong:
    ...
    def rename(self, rename_dict):
        ...
        for col_cur, col_new in rename_dict.items():
            if col_cur in self.data.columns:
                if col_cur in ['m', 'j']:
                    self.data = self.data.rename({col_cur: col_new}, axis=1)
                    self.col_dict[col_cur] = None
                    if col_cur == 'j':
                        self.contiguous_cids = None
                else:
...
class BipartiteEventStudy:
    ...
    def rename(self, rename_dict):
        ...
        '''
        for col_cur, col_new in rename_dict.items():
            if col_cur in self.data.columns or (col_cur == 'weight' and self.col_dict['w1'] is not None and self.col_dict['w2'] is not None) or (col_cur == 'j' and self.col_dict['j1'] is not None and self.col_dict['j2'] is not None):
                if col_cur == 'm':
                    self.data = self.data.rename({col_cur: col_new}, axis=1)
                    self.col_dict[col_cur] = None
                elif col_cur == 'weight':
                    self.data = self.data.rename({'w1': col_new + '1', 'w2': col_new + '2'}, axis=1)
                    self.col_dict['w1'] = None
                    self.col_dict['w2'] = None
                elif col_cur == 'j':
                    self.data = self.data.rename({'j1': col_new + '1', 'j2': col_new + '2'}, axis=1)
                    self.col_dict['j1'] = None
                    self.col_dict['j2'] = None
                    self.contiguous_cids = None
                else:
