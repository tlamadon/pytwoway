...
class vcycle():
    ...
    def __init__(self, A, shape, num_pre_iter=1, num_post_iter=1, omega=0.8, direct_solve_size=64, cache=None):
        if cache is None:
            cache = {}

        self.A = A
        self.shape = shape
        self.num_pre_iter = num_pre_iter
        self.num_post_iter = num_post_iter
        self.omega = omega
        self.direct_solve_size = direct_solve_size
        self.cache = cache

    def precondition(self, r):
        return _vcycle_step(
            self.A, r, self.shape, self.cache, self.num_pre_iter, self.num_post_iter, self.omega, self.direct_solve_size
        )
